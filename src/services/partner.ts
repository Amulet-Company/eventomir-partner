...
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { apiRequest } from "@/utils/api-client";
import { uploadFileDirectly } from "@/utils/uploadService";
...
/**
 * Updates the partner profile.
 * Now utilizes Direct-to-Cloud MinIO uploads for files and sends a standard JSON payload.
 */
const updatePartnerProfileFn = async ({
  userId,
  data,
  ...
  let backgroundPicUrl: string | undefined = undefined;

  // 1. Upload Profile Picture directly to MinIO if provided
  if (data.profilePictureFile) {
    // "profileImage" must match the ALLOWED_FOLDERS in your Node.js backend exactly
  ...
  // 2. Upload Background Picture directly to MinIO if provided
  if (data.backgroundPictureFile) {
    // "backgroundPicture" must match the ALLOWED_FOLDERS in your Node.js backend exactly
    const uploadResult = await uploadFileDirectly(
      data.backgroundPictureFile,
      "backgroundPicture",
    );
    backgroundPicUrl = uploadResult.publicUrl;
  }

  // 3. Construct the clean JSON payload
  const payload = {
    name: data.name,
    phone: data.phone,
    ...
    inn: data.inn,
    socialLinks: data.socialLinks, // Send standard objects; no need for JSON.stringify() anymore
    bankDetails: data.bankDetails,
    image: profilePicUrl,
    backgroundPicture: backgroundPicUrl,
  };

  // 4. Clean up undefined fields so we don't accidentally overwrite existing backend data with nulls
  const cleanPayload = Object.fromEntries(
    Object.entries(payload).filter(([_, value]) => value !== undefined),
  );

  // 5. Send standard JSON request to your updated backend route
  return await apiRequest<{ message: string; profile: any }>({
    method: "patch",
    url: `/api/partners/${userId}`,
    data: cleanPayload,
